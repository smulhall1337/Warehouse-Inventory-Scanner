...
import gui.MyTableModel.ColumnListener;
...
public class ReportsWindow {
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					...
					window.frmWimsReports.setVisible(true);
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
	...
	{
		frmWimsReports = new JFrame();
		frmWimsReports.setTitle("WIMS - Reports");
		frmWimsReports.setBounds(100, 100, 718, 534);
		frmWimsReports.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		frmWimsReports.getContentPane().setLayout(new BorderLayout(0, 0));
	}
	
	private void initializeReportSelectionOptionsPanel() {
		reportSelectionOptionsPanel = new JPanel();
		frmWimsReports.getContentPane().add(reportSelectionOptionsPanel, BorderLayout.NORTH);
	...
	private void initializeGenerateButton(){
		...
		generateButtonPanel.add(btnNewButton);
	...
	private void displayEmployeeReport() {
		...
		{
			JOptionPane.showMessageDialog(frmWimsReports, "An unexpected database error occured when " 
	...
	private void populateTable(List<String[]> dataList, String[] columnNames) {
        ...
        dataArrays = (String[][]) dataList.toArray(dataArrays);
	...
	{
		reportTableScrollPane = new JScrollPane();
		frmWimsReports.getContentPane().add(reportTableScrollPane, BorderLayout.CENTER);
		...
		Object[][] defaultData = new Object[52][50]; 
		String[] defaultColNames = new String[52];
...
//		    header.setReorderingAllowed(true);
		...
		table.getColumnModel().getColumn(14).setMinWidth(50);
		reportTableScrollPane.setViewportView(table);
	}

	private void initializeExport(){
		JPanel exportPanel = new JPanel();
		frmWimsReports.getContentPane().add(exportPanel, BorderLayout.SOUTH);
	...
	private void initializeBorderStruts()
	{
		...
		frmWimsReports.getContentPane().add(horizontalStrut, BorderLayout.WEST);
		
		Component horizontalStrut_1 = Box.createHorizontalStrut(20);
		frmWimsReports.getContentPane().add(horizontalStrut_1, BorderLayout.EAST);
	}
}
